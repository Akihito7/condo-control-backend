...
export class AuthService {
  ...
  async me(userId: number) {
    const resultUsers = await this.supabase
      .from('user')
      .select(
        `*,
        user_association (*)
    ...
    if (!user.is_super) {
      ...
      const pagesWithPermissionByRoleFormatted = camelcaseKeys(
        pagesWithPermissionByRole?.map((page) => flattenObject(page)) ?? [],
      );
      const modulesWithPermissionByRoleFormatted = camelcaseKeys(
        modulesWithPermissionByRole?.map((module) => flattenObject(module)) ??
          [],
      );
      ...
      const tabStructure = modulesWithPermissionByRoleFormatted
        ...
        .map((module: any) => {
          const pagesToThisModule = pagesWithPermissionByRoleFormatted.filter(
            (page: any) => page.pageModuleId === module.moduleId && !!page.read,
          );
          const pagesToThisPlan = pagesToThisModule.filter((page: any) =>
            pages?.some((pagePlan) => pagePlan.page_id === page.pageId),
          );
          return {
      ...
      const normalizedData = {
        ...
        condominiumLogo: currentCondominium.logo,
      ...
      const userObjectFlatten = camelcaseKeys(
        flattenObject(normalizedData),
      ) as User;
      return userObjectFlatten;
    }

    const { data: superModule, error: errorSuperModules } = await this.supabase
      .from('module')
      .select('*, page (*)')
      .eq('is_super', true);
    ...
    const normalizedData = {
      ...user,
      condominiumId: null,
      condominiumLogo: null,
      pagesWithPermissionByRole: [],
      modulesWithPermissionByRole: [],
      tabStructure,
    };

    const userObjectFlatten = camelcaseKeys(flattenObject(normalizedData), {
      deep: true,
    }) as User;
    return userObjectFlatten;
  }
  ...
  async resetPassword({ password, code }: { password: string; code: string }) {
    ...
    const { error: updateCodeError } = await this.supabase
      .from('confirmation_codes')
      .update({
        used: true,
      })
