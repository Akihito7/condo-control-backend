...
export class CommunicationService {
  ...
  async getCardsOpeningOfCalls(data: ParamOpeningCalls) {
    ...
    accuracyHoursCallSolved =
      totalHoursCallSolved / totalCallsSolved > 0
        ? totalHoursCallSolved / totalCallsSolved
        : 0;
  ...
  async getRecordsOpeningOfCalls(data: ParamOpeningCalls) {
    ...
    const flattenCallRecords = callRecords.map((callRecord) =>
      flattenObject(callRecord),
    );
    ...
    const callsWithAttachments = flattenCallRecords.map((call: any) => {
      return {
        ...call,
        attachments: camelkeysAttachments?.filter(
          (att: any) => att.relatedId === call.id,
        ),
      };
    });
  ...
  async createOpeningCallRecord(
    ...
    if (Array.isArray(attachment) && attachment.length > 0) {
      attachment.map(async (attachment) => {
        ...
        const { data, error: attachmentInserError } = await this.supabase
          .from('attachments')
          .insert({
            related_type: 'calls',
            related_id: recordId,
  ...
  async createAttachmentOpeningCalls(
    ...
    if (Array.isArray(attachment) && attachment.length > 0) {
      const result = attachment.map(async (attachment) => {
        ...
        const { data: attachments, error: attachmentInserError } =
          await this.supabase
            .from('attachments')
            .insert({
              related_type: 'calls',
  ...
  async deleteAttachmentOpeningCall(attachmentId: string) {
    ...
    const { data: dataDelete, error: errorDelete } = await this.supabase
      .from('attachments')
      .delete()
      .eq('id', attachmentId);
    ...
    const { data, error: errorDeleteAttachment } = await this.supabase.storage
      .from('condo')
      .remove([relativePath]);
  ...
  async deleteOpeningCallsRecord(recordId: string) {
    ...
    const attachmentsToDelete = attachments.map(async (attachment) => {
      const { data: _, error: errorDelete } = await this.supabase
        .from('attachments')
        .delete()
        .eq('id', attachment.id);
  ...
  async updateOpeningCallRecord(
    recordId: string,
    data: Omit<BodyOpeningCalls, 'date'>,
  ) {
    const {
      description,
      issueTypeId,
  ...
  async getDeliveriesByCondominiumId(filters: any) {
    ...
    return deliveriesCamalcase.map((delivery: any) => ({
      ...delivery,
      attachments: attachmentsCamelcase?.filter(
        (att: any) => att.relatedId === delivery.id,
      ),
    }));
  }
  ...
  async createDelivery(data, attachment) {
    ...
    if (!apartamentId) {
      throw new BadRequestException(
        'Nenhum apartamento foi encontrado com esse número.',
      );
    }
  ...
  async getAssemblyVirtualPolls(
    ...
    const { startDate, endDate } = getFullMonthInterval(filters.date);
    ...
    const pollsWithVote = await Promise.all(
      polls?.map(async (poll) => {
        const { data: votes, error: votesError } = await this.supabase.rpc(
          'get_poll_votes',
          { p_poll_id: poll.id },
        );
        ...
        const votesFormatted = camelcaseKeys(
          votes.map((vote) => flattenObject(vote)),
        ) as any;
        ...
        votesFormatted.forEach((vote: any) => {
          const hasVoteInfo = votesInfo.find(
            (voteInfo) => voteInfo.optionId === vote.optionId,
          );
          ...
          if (!hasVoteInfo) {
            ...
            const indexCurrentVote = votesFormatted.findIndex(
              (vote) => vote.optionId === hasVoteInfo.optionId,
            );
        ...
        const currentUserAlreadyVoted = votes.some(
          (vote) => vote.polls_user_relation.user_id === userId,
        );
        const currentVoteUser = votesFormatted.find(
          (vote) => vote.userId === userId,
        )
          ? votesFormatted.find((vote) => vote.userId === userId).pollsOptionsId
          : null;
        const currentVoteUserId = votesFormatted.find(
          (vote) => vote.userId === userId,
        )
          ? votesFormatted.find((vote) => vote.userId === userId)
              .pollsUserRelationId
          : null;
        const percentageParticipation = (
          (uniquesApartamentsIds.size / apartaments.length) *
          100
        ).toFixed(2);
        return {
          ...poll,
    ...
    const reducePercetageParticipation = pollsWithVote.reduce(
      (acc, vote: any) => {
    ...
    const accuracyPercentageParticipation =
      reducePercetageParticipation / polls.length;
  ...
  async createVoteAssemblyVirtualPoll(
    ...
    const alreadyFinished = isBefore(
      new Date(currentPoll.end_date),
      new Date(),
    );
    if (alreadyFinished) {
  ...
  async createAssemblyVirtualPoll(data: any, token: string) {
    ...
    if (pollIdInserted) {
      await Promise.all(
        data.options.map(async (option) => {
          return this.supabase.from('polls_options').insert({
            name: option.name,
  ...
  async updateAssemblyVirtualPoll(pollId: string, data: any, token: string) {
    ...
    if (data.optionsToRemove.length > 0) {
      await Promise.all(
        data.optionsToRemove.map(async (option) => {
          await this.supabase
            .from('polls_user_relation')
            .delete()
            .eq('option_id', option);
  ...
  async getScheduleCondomnium({
    ...
    const { endDate } = getFullMonthInterval(date);
    ...
    for (let i = 0; i < Number(endDay); i++) {
      ...
      const eventsFiltered = eventsCamelkeys?.filter((event) =>
        event.startTime.includes(dateFormattedToDay),
      );
      const dayName = format(`${dateFormattedToDay}T00:00:00`, 'EEEE', {
        locale: ptBR,
      });
  ...
  async finishPoll() {
    ...
    await Promise.all(
      pollsToFinished?.map(async (poll) => {
  ...
  async sendCodeToMarkAsDelivery(deliveryId: string) {
    ...
    if (!mainEmail) {
      throw new Error(
        'Não há e-mail vinculado a este apartamento. Por favor, entre em contato com o síndico.',
      );
    }

    // Insere o código de confirmação
    const { data: codeInserteds, error: insertCodeError } = await this.supabase
  ...
  async createResidentRequest(
    ...
    const currentRegisterInserted = dataInserted?.[0];

    const attachmentsPromisses = attachments.map(async (attachament) => {
      const fileNameNormalized = normalizeFileName(attachament.originalname);
      const uniqueFileName = `${v4()}-${fileNameNormalized}`;

      const { data, error } = await this.supabase.storage
        .from('condo')
        .upload(uniqueFileName, attachament.buffer);

      if (error) throw new Error(error.message);

      const { fullPath, path, id } = data;

      const { error: insertFileError } = await this.supabase
        .from('attachments')
        .insert({
          related_type: 'resident_calls',
          related_id: currentRegisterInserted.id,
          condominium_id: condominiumId,
          date: format(new Date(), 'yyyy-MM-dd'),
          path: fullPath,
          bucket_name: 'condo',
          original_name: attachament.originalname,
          created_at: new Date(),
          supabase_id: id,
        })
        .select('*');
    ...
    await Promise.all(attachmentsPromisses);
  }

  async getGravities() {
    const { data, error } = await this.supabase
      .from('resident_call_gravity')
      .select('*');
